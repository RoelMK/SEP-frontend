<template>
  <v-card>
      ...
      <v-dialog v-model="dialog" scrollable max-width="600px">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="primary darken-1" @click="dialog = false; request()">
              Request
            </v-btn>
          </v-card-actions>
        </v-card>
    ...
    <v-row class="mx-2 temporaryPadding" align="center" v-for="supervisor in supervisors" :key="supervisor.supervisor_email">
        <v-col
            cols="9"
        >
  ...
  methods: {
    request() {
        Supervisor.request({ 
            ...
            .then(
                (resp) => { this.childEmail = ""; console.log(resp); },
    ...
    getRequested() {
        Supervisor.getSupervisors({
            ...
            .then(
                (resp) => { this.requestedSupervisors = resp.data.supervisors },
                (error) => { console.log(error) }
            )
    },
    approve(supervisor) {
        Supervisor.request({
            ...
            .then(
                () => { this.requestedSupervisors.splice(this.requestedSupervisors.indexOf(supervisor), 1) },
                (error) => { console.log(error) }      
            )
    },
    reject(supervisor) {
        Supervisor.deleteSupervisor({
            ...
            .then(
                (resp) => { this.requestedSupervisors.splice(this.requestedSupervisors.indexOf(supervisor), 1);console.log(resp) },
                (error) => { console.log(error) }
            )
    },
    getApproved() {
        Supervisor.getApproved({
            ...
            .then(
                (resp) => { this.supervisors = resp.data.supervisors },
                (error) => { console.log(error) }
            )
    },
    remove(supervisor) {
        Supervisor.deleteSupervisor({
            ...
            .then(
                (resp) => { this.supervisors.splice(this.supervisors.indexOf(supervisor), 1);console.log(resp) },
                (error) => { console.log(error) }
            )
    },
